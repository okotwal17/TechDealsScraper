...
def scrape_ebay(query):
    """
    Scrape eBay for products based on the search query.
    ...
    mock_data = [
        {
            'website': 'eBay',
            'title': f'Apple MacBook Pro 13" (Mock Data)',
            'price': '$999.99',
            'url': 'https://www.ebay.com/sch/i.html?_nkw=macbook+pro'
        },
        {
            'website': 'eBay',
            'title': f'Apple MacBook Pro 15" (Mock Data)',
            'price': '$1,299.99',
            'url': 'https://www.ebay.com/sch/i.html?_nkw=macbook+pro'
        },
        {
            'website': 'eBay',
            'title': f'Apple MacBook Pro 16" (Mock Data)',
            'price': '$1,799.99',
            'url': 'https://www.ebay.com/sch/i.html?_nkw=macbook+pro'
        }
    ]
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"eBay scraping attempt {attempt + 1} failed: {e}")
        ...
        for listing in listings[:10]:  # Limit to first 10 results
            # Skip "More items like this" listing
            if listing.select_one('.s-item__title--tagblock'):
                continue
            ...
            # Extract product URL
            url_element = listing.select_one('.s-item__link')
            if not url_element:
                continue
            ...
            # Clean up price text (remove "to" in price ranges, get the lower price)
            if ' to ' in price_text:
